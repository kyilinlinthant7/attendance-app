...
$(document).on('change', '.dateu', function () {
// $('#getdata').on('change', function () {
    ...
    // var new_date_to = new Date(date_to);
    // var timeDiff = Math.abs(new_date_to.getTime() - new_date_from.getTime());
    // var diffD = Math.ceil(timeDiff / (1000 * 3600 * 24))+1;
...
$('.confirmClick').click(function () {
 ...
 $.post('/confirm-leave', {'leaveId': leave_id, '_token' : token}, function(data)
 {
 ...
 leave_button.empty();
 leave_button.append("<button type='button' class='btn btn-warning br2 btn-xs fs12' aria-expanded='false'><i class='fa fa-check'> Confirm </i> </button>");
 setTimeout(function() {
